...
class BinanceFutureHttp(object):
    ...
    def __init__(self, api_key=None, secret=None, domain=None, proxy_host="", proxy_port=0, timeout=5, try_counts=5):
        self.key = api_key
        self.secret = secret
        self.domain = domain
    ...
    def request(self, req_method: RequestMethod, path: str, requery_dict=None, verify=False):
        url = f"https://fapi.{self.domain}{path}" 
...
if __name__ == '__main__':
    ...
    secret = 'xxxx'
    domain = 'binance.com'
    binance = BinanceFutureHttp(domain=domain, api_key=key, secret=secret)
